...
/* 처방 탭 "필터 초기화" 버튼에서 호출 — 인물 셰이더 포커스까지 전체 해제 */
window.__prescBodyViewerReset = function () {
  selectPart('', 'reset');
};

/* ── 처방 탭 활성화 감지 ─────────────────────────────────── */
let _inited = false;
/* 모바일(≤700px)에서는 처방 탭이 PC 최적화 안내 화면으로 대체되므로
   무거운 인터랙티브 인체 뷰어를 초기화하지 않는다. */
const _isMobile = () => window.matchMedia('(max-width: 700px)').matches;
function tryInit() {
  const bv = document.getElementById('body-view');
  if (!bv) return;
  new MutationObserver(() => {
    if (bv.getAttribute('aria-hidden') === 'false' && !_inited && !_isMobile()) { _inited = true; init(); }
  }).observe(bv, { attributes: true, attributeFilter: ['aria-hidden'] });
  if (bv.getAttribute('aria-hidden') === 'false' && !_isMobile()) { _inited = true; init(); }
}
