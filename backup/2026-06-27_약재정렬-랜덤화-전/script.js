  ...
  function renderOriginView() {
    ...
    ORIGIN_ORDER.forEach(function (key) {
      ...
      item.addEventListener('click', function () {
        // 드릴(2depth)·트리맵에서 현재 출신이 아닌 다른 출신을 누르면 그 출신엔 0건이라
        // 아무것도 안 보인다 → 해당 출신의 2depth(드릴)로 이동시킨다.
        if (viewState === 'drill' || viewState === 'treemap') {
    ...
    function showTreemap(d, opts) {
      ...
      if (opts && opts.demo) startFilterDemo(450, presentOrigins(d.data.breakdown));
    }
