(function () {
  ...
  function renderRareHerbsSection() {
    ...
    var RARE_HERBS = [
      ...
      { name: '유황', hanja: '硫黃', english: 'Sulfur', img: 'asset/img/Sulfur.png',
        desc: '화산에서 난 순수한 황. 양기를 북돋고 냉증·무릎 통증에 썼다.',
      ...
      { name: '노사', hanja: '磠砂', english: 'Sal Ammoniac', img: 'asset/img/Sal_ammoniac.png',
        desc: '천연 염화암모늄 결정. 어혈·군살·종양을 삭인 부식성 광물.',
        reason: '강한 부식성으로 위장 점막을 손상시키고, 다량 복용 시 암모니아 중독을 일으킬 수 있다는 것이 확인되어 내복 처방에서 퇴출되었다.' },
      ...
      { name: '이색', hanja: '耳塞', english: 'Earwax', img: 'asset/img/Earwax.png',
        desc: '사람의 귀지. 눈 이물질이나 벌레 물린 데 바르던 민간 약.',
    ...
    /* ── 31종 중 "겹치는 사유"를 덜어낸 핵심 20종 (개별 약재 카드) ──
    ...
    var FEATURED = { '정화수': 1, '야명사': 1, '천령개': 1, '빈랑': 1, '우박': 1, '진육': 1 };
    ...
    // 마지막 카드(노사) 활성 시 우측 "이유" 카드가 화면 끝에 붙지 않도록 스크롤 여유 확보
  ...
  function initHorizontalTimeline() {
    ...
    var current = -1;
    var autoTimer = null;
    var userTook = false;   // 사용자가 직접 클릭하면 자동재생 영구 종료
    var inView = false;
    ...
    function setActive(idx) {
      ...
      details.forEach(function (d) {
        d.classList.toggle('active', d.getAttribute('data-htl-idx') === String(idx));
      });
      var node = nodes[idx];
      if (node && scroller) {
        scroller.scrollTo({
    ...
    function startAuto() {
      if (autoTimer || userTook || !inView) return;
