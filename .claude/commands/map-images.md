...
```bash
# 현재 매핑된 파일명 추출 — 단/이중 따옴표 경로 모두 처리
python3 -c "
import re
with open('js/herb-viewer.js') as f:
    content = f.read()
paths = set()
paths.update(re.findall(r'\"asset/Img/([^\"]+)\"', content))
paths.update(re.findall(r\"'asset/Img/([^']+)'\", content))
for p in sorted(paths):
    print(p)
" > /tmp/mapped.txt

# asset/img 전체 파일 목록 (NO_*.png 제외) — find 사용으로 특수문자 파일명 안전 처리
find asset/img -name "*.png" ! -name "NO_*" -exec basename {} \; | sort > /tmp/all_images.txt
...
// 파일명에 작은따옴표 포함된 경우 (예: Dragon's-blood.png)
'HERB_ID': "asset/Img/Filename's-name.png",
```
...
- `THUMBNAIL_REFERENCE.md`를 Read 도구로 읽지 말 것 — shell grep으로 대체
- `herb-viewer.js`는 수정 시 1회만 Read (끝부분 확인 시 offset 460 이후)
- `NO_*.png` 파일은 매핑 대상 아님 (단, `PLANT_292` 등 이미 매핑된 NO_ 항목은 유지)
- 파일 목록 추출은 반드시 `find`를 사용한다 — `ls | xargs`는 작은따옴표 파일명에서 오류 발생
- 매핑 추출은 반드시 Python으로 단/이중 따옴표를 모두 처리한다 — `grep -oP "asset/Img/\K[^']+"` 방식은 이중따옴표 경로를 누락함

## 파일 구조 참고

- 매핑 오브젝트: `js/herb-viewer.js` 상단 `var THUMBNAIL_BY_ID = { ... };`
- 경로 형식: `'HERB_ID': 'asset/Img/Filename.png'` (파일명에 `'` 포함 시 이중따옴표 사용)
